 ...
 * <p>
 * The {@link EnumerableRel} operators are used to generate Java code. The code is compiled by an
 * embedded Java compiler (Janino) in order to process the input data and generate the results. In
 * order to "glue" together the {@link LuceneRel} operators with {@link EnumerableRel} operators
 * this converter needs to generate java code calling the Lucene APIs.
 *
...
public final class LuceneToEnumerableConverter extends ConverterImpl implements EnumerableRel {
  ...
  public LuceneToEnumerableConverter(RelNode child) {
    super(child.getCluster(), ConventionTraitDef.INSTANCE,
        child.getCluster().traitSet().replace(EnumerableConvention.INSTANCE), child);
  }
  ...
  public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
    try {
      ...
      for (RelDataTypeField f : getRowType().getFieldList()) {
        ...
        MethodCallExpression callPut = Expressions.call(fieldStmt.parameter, putMethod, fieldName, fieldType);
        codeBlock.add(Expressions.statement(callPut));
      }
      ConstantExpression indexPath = Expressions.constant(((LuceneRel) input).implement().indexPath);
      ConstantExpression luceneQuery = Expressions.constant(((LuceneRel) input).implement().query.toString());
      ...
      PhysType physType = PhysTypeImpl.of(implementor.getTypeFactory(), getRowType(), pref.prefer(JavaRowFormat.ARRAY));
